...
def parse_and_insert_json(json_file,Bad_Artists):
    ...
    for _, row in df.iterrows():
        ...
        upsert_song(row["Timestamp"], row["username"],row["song_name"], row["artist_name"], row["real_nigga?"])
...
def upsert_song(timestamp, username, song, artist, real_nigga=None):
    ...
    if existing_entry:
        ...
        if existing_song != song or existing_artist != artist:
            update_query = """
            UPDATE songs
            SET date_created = ?, song = ?, artist = ?, "real_nigga" = ?
            WHERE id = ?;
            """
            conn.execute(update_query, [formatted_timestamp, song, artist, real_nigga, existing_id])
        ...
        # Insert a new row
        insert_query = """
        INSERT INTO songs (date_created, username, song, artist, "real_nigga")
        VALUES (?, ?, ?, ?, ?);
        """
        conn.execute(insert_query, [formatted_timestamp, username, song, artist, real_nigga])
